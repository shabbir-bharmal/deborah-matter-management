...
- The `<laravel-boost-guidelines>` block below is generated by `php artisan boost:update`, which runs automatically on `composer update`. It rewrites only that block in place and preserves everything outside it — keep hand-written guidance outside the block, never inside it.
- `CLAUDE.md` is a byte-identical copy of this file. After manual edits here, sync it: `cp AGENTS.md CLAUDE.md`.
