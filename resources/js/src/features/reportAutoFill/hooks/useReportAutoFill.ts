...
import { emptyReportDraft, REPORT_SECTION_KEYS, type MappedField, type ReportDraft, type ReportSectionKey } from '../types/reportAutoFill';
...
export interface ReportAutoFillState {
    ...
    clearRejected: () => void;
    updateFieldValue: (sectionKey: ReportSectionKey, index: number, value: string) => void;
    clearField: (sectionKey: ReportSectionKey, index: number) => void;
}
...
export function useReportAutoFill(matterId: string): ReportAutoFillState {
    ...
    const updateFieldValue = useCallback((sectionKey: ReportSectionKey, index: number, value: string) => {
        setDraft(
            (current) =>
                current &&
                withSection(current, sectionKey, (fields) =>
                    fields.map((field, fieldIndex) => (fieldIndex === index ? { ...field, value, edited: true } : field)),
                ),
        );
    }, []);

    const clearField = useCallback((sectionKey: ReportSectionKey, index: number) => {
        setDraft((current) => current && withSection(current, sectionKey, (fields) => fields.filter((_, i) => i !== index)));
    }, []);

    return { files, rejected, draft, addFiles, removeFile, clearRejected, updateFieldValue, clearField };
}

function withSection(draft: ReportDraft, sectionKey: ReportSectionKey, transform: (fields: MappedField[]) => MappedField[]): ReportDraft {
