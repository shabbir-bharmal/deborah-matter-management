...
test.describe('route smoke tests', () => {
    ...
    for (const [url, title] of [
        ['/matters/inv-001', 'Harassment allegations — Engineering department'],
        ['/matters/inv-002', 'Discrimination claim — promotion process'],
        ['/matters/inv-003', 'Expense report misconduct — regional sales'],
    ] as const) {
    ...
    test('deep-links to workspace tab /matters/inv-001/timeline with real content', async ({ page }) => {
        await page.goto('/matters/inv-001/timeline');
        await expect(page.getByText('Complaint received').first()).toBeVisible();
    });

    test('deep-links to workspace tab /matters/inv-001/interviews with real content', async ({ page }) => {
        await page.goto('/matters/inv-001/interviews');
        await expect(page.getByText('Sarah Okafor').first()).toBeVisible();
    });

    test('deep-links to workspace tab /matters/inv-001/evidence with real content', async ({ page }) => {
        await page.goto('/matters/inv-001/evidence');
        await expect(page.getByText(/Stand-up meeting recording/i).first()).toBeVisible();
    });

    test('deep-links to workspace tab /matters/inv-001/findings with real content', async ({ page }) => {
        await page.goto('/matters/inv-001/findings');
        await expect(page.getByText('Supporting evidence').first()).toBeVisible();
    });

    test('deep-links to workspace tab /matters/inv-001/documents with real content', async ({ page }) => {
        await page.goto('/matters/inv-001/documents');
        await expect(page.getByText('Interview transcript — Sarah Okafor')).toBeVisible();
    });

    test('deep-links to workspace tab /matters/inv-001/reports with real content', async ({ page }) => {
    ...
    test('shows matter not found for an unknown investigation id', async ({ page }) => {
        await page.goto('/matters/does-not-exist');
